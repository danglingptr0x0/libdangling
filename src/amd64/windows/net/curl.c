...
static size_t ldg_curl_resp_wr_cb(void *contents, size_t size, size_t nmemb, void *user_data)
{
    ...
    if (!resp->data || new_len + LDG_STR_TERM_SIZE > resp->cap)
    {
        ...
        if (resp->data)
        {
            if (LDG_UNLIKELY(ldg_mem_secure_copy(new_data, resp->data, resp->size) != LDG_ERR_AOK)) { _aligned_free(new_data); return 0; }
...
uint32_t ldg_curl_multi_ctx_create(ldg_curl_multi_ctx_t *ctx, uint64_t cap)
{
    ...
    reqs_tmp = _aligned_malloc((size_t)(cap * sizeof(ldg_curl_multi_req_t)), LDG_AMD64_CACHE_LINE_WIDTH);
    ...
    if (LDG_UNLIKELY(memset(ctx->reqs, 0, cap * sizeof(ldg_curl_multi_req_t)) != ctx->reqs)) { curl_multi_cleanup(ctx->multi); _aligned_free(ctx->reqs); return LDG_ERR_MEM_BAD; }
