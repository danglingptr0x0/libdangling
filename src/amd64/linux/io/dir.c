...
uint32_t ldg_io_dir_create(const char *path, uint32_t mode)
{
    ...
    if (LDG_UNLIKELY(mkdir(path, (mode_t)mode) < 0)) { return ldg_io_dir_errno_translate(); }
...
uint32_t ldg_io_dir_destroy(const char *path)
{
    ...
    if (LDG_UNLIKELY(rmdir(path) < 0)) { return ldg_io_dir_errno_translate(); }
...
uint32_t ldg_io_dir_open(const char *path, ldg_io_dir_t **out)
{
    ...
    *out = 0x0;
    ...
    if (d->path_len > 0 && d->path[d->path_len - 1] != '/')
    {
        d->path[d->path_len] = '/';
        d->path_len++;
...
uint32_t ldg_io_dir_rd(ldg_io_dir_t *d, ldg_io_dirent_t **out)
{
    ...
    *out = 0x0;
    ...
    if (d->path_len + name_len < full_path_size)
    {
        ...
        if (lstat(full_path, &sb) == 0) { e->is_dir = S_ISDIR(sb.st_mode) ? 1 : 0; }
    }
