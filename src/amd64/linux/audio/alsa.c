...
// singleton audio backend; file-scope static required for ALSA state
...
static uint32_t audio_mixer_open(int32_t card_idx)
{
    ...
    ret = (int32_t)snd_mixer_open(&g_audio_ctx.mixer, 0);
    if (LDG_UNLIKELY(ret < 0)) { return LDG_ERR_AUDIO_INIT; }

    ret = (int32_t)snd_mixer_attach(g_audio_ctx.mixer, card_name);
    ...
    ret = (int32_t)snd_mixer_selem_register(g_audio_ctx.mixer, 0x0, 0x0);
    if (LDG_UNLIKELY(ret < 0))
    {
        snd_mixer_close(g_audio_ctx.mixer);
        g_audio_ctx.mixer = 0x0;
    ...
    ret = (int32_t)snd_mixer_load(g_audio_ctx.mixer);
    if (LDG_UNLIKELY(ret < 0))
    {
        snd_mixer_close(g_audio_ctx.mixer);
        g_audio_ctx.mixer = 0x0;
...

    {
        ...
        if (LDG_UNLIKELY(snd_mixer_selem_get_playback_volume_range(g_audio_ctx.master_elem, &alsa_min, &alsa_max) < 0)) { return LDG_ERR_AUDIO_INIT; }

        g_audio_ctx.vol_min = (int64_t)alsa_min;
        g_audio_ctx.vol_max = (int64_t)alsa_max;
    }
...
uint32_t ldg_audio_init(void)
{
    ...
    if (card_env) { card_idx = (int32_t)strtol(card_env, 0x0, LDG_BASE_DEC); }
...
uint32_t ldg_audio_master_vol_get(double *vol)
{
    ...
    ret = (int32_t)snd_mixer_selem_get_playback_volume(g_audio_ctx.master_elem, SND_MIXER_SCHN_FRONT_LEFT, &alsa_val);
    if (LDG_UNLIKELY(ret < 0)) { return LDG_ERR_AUDIO_INIT; }

    val = (int64_t)alsa_val;
...
uint32_t ldg_audio_stream_vol_get(__attribute__((unused)) uint32_t stream_id, __attribute__((unused)) double *vol)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}

uint32_t ldg_audio_stream_vol_set(__attribute__((unused)) uint32_t stream_id, __attribute__((unused)) double vol)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}
...
uint32_t ldg_audio_stream_mute_get(uint32_t stream_id, uint8_t *muted)
{
...

    {
        ...
        ret = (int32_t)snd_mixer_selem_get_playback_switch(g_audio_ctx.master_elem, SND_MIXER_SCHN_FRONT_LEFT, &alsa_val);
        if (LDG_UNLIKELY(ret < 0)) { return LDG_ERR_AUDIO_INIT; }

        val = (int32_t)alsa_val;
    }
...
uint32_t ldg_audio_stream_mute_set(uint32_t stream_id, uint8_t muted)
{
    ...
    ret = (int32_t)snd_mixer_selem_set_playback_switch_all(g_audio_ctx.master_elem, muted ? 0 : 1);
    if (LDG_UNLIKELY(ret < 0)) { return LDG_ERR_AUDIO_INIT; }

    return LDG_ERR_AOK;
}

uint32_t ldg_audio_stream_list(__attribute__((unused)) ldg_audio_stream_t **streams, __attribute__((unused)) uint32_t *cunt)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}

void ldg_audio_stream_free(__attribute__((unused)) ldg_audio_stream_t *streams)
{
}

uint32_t ldg_audio_stream_name_get(__attribute__((unused)) const char *name, __attribute__((unused)) ldg_audio_stream_t *stream)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}

void ldg_audio_self_register(__attribute__((unused)) uint32_t stream_id)
{
}
...
uint32_t ldg_audio_stream_self_get(__attribute__((unused)) ldg_audio_stream_t *stream)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}

uint32_t ldg_audio_self_vol_get(__attribute__((unused)) double *vol)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}

uint32_t ldg_audio_self_vol_set(__attribute__((unused)) double vol)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}
...
uint32_t ldg_audio_src_list(ldg_audio_src_t **srcs, uint32_t *cunt)
{
    ...
    arr = (ldg_audio_src_t *)ldg_mem_alloc(n * sizeof(ldg_audio_src_t));
    if (LDG_UNLIKELY(!arr)) { return LDG_ERR_ALLOC_NULL; }

    if (LDG_UNLIKELY(memset(arr, 0, n * sizeof(ldg_audio_src_t)) != arr)) { ldg_mem_dealloc(arr); return LDG_ERR_MEM_BAD; }
...
uint32_t ldg_audio_default_sink_get(ldg_audio_sink_t *sink)
{
    ...
    if (LDG_UNLIKELY(ldg_audio_master_vol_get(&sink->vol) != LDG_ERR_AOK)) { sink->vol = 0.0; }
...
uint32_t ldg_audio_default_src_get(ldg_audio_src_t *src)
{
    ...
    if (LDG_UNLIKELY(memset(src, 0, sizeof(ldg_audio_src_t)) != src)) { snd_ctl_close(ctl); return LDG_ERR_MEM_BAD; }

    src->id = (uint32_t)g_audio_ctx.card_idx;
    if (LDG_UNLIKELY(ldg_strrbrcpy(src->name, snd_ctl_card_info_get_id(info), LDG_AUDIO_NAME_MAX) > LDG_ERR_STR_OVERLAP)) { snd_ctl_close(ctl); return LDG_ERR_MEM_BAD; }

    if (LDG_UNLIKELY(ldg_strrbrcpy(src->desc, snd_ctl_card_info_get_name(info), LDG_AUDIO_DESC_MAX) > LDG_ERR_STR_OVERLAP)) { snd_ctl_close(ctl); return LDG_ERR_MEM_BAD; }
...
uint32_t ldg_audio_duck(__attribute__((unused)) double factor, __attribute__((unused)) uint32_t exclude_stream_id)
{
    return LDG_ERR_AUDIO_NOT_AVAILABLE;
}
